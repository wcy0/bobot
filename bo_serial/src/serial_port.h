...
private:
	...
	enum STATE {
		WAITING_FF,GET_MODE,MUT_REG_RETURN,SINGLE_REG_RETURN,READ_REG,CHECKSUM,OVER
	} m_state;								// 程序工作状态
	...
	function<void(bo_msgs::bo_DataGramPtr)> m_dataCallbackFunc;		// 数据回调函数
...
public:
	...
	// 设置收到数据之后的回调函数
	void setCallbackFunc(const function<void(bo_msgs::bo_DataGramPtr)> &func);

	// 向串口中发送一个数据报文
	bool writeDataGram(const bo_msgs::bo_DataGram &datagram);
	// 向串口中直接写入一串数据
